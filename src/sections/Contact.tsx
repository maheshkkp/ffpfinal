...
import emailjs from '@emailjs/browser'; // Keep emailjs import

const Contact: React.FC = () => {
  const form = useRef<HTMLFormElement>(null); // Keep the ref for the form

  const sendEmail = (e: React.FormEvent) => {
    e.preventDefault(); // Keep prevention of default form submission

    if (form.current) {
      emailjs.sendForm('hostinger_smtp', 'template_p6yax4o', form.current, '9p8TjlXgZCWDiT920')
          .then((result) => {
            console.log(result.text);
            alert('Message sent successfully!');
            form.current?.reset(); // Keep form reset
          }, (error) => {
      ...
      <section id="contact" className="py-20">
        <div className="container">
          ...
          <div className="mt-16 max-w-3xl mx-auto">
            ...
            <form ref={form} onSubmit={sendEmail} className="space-y-6"> {/* Add ref and onSubmit here */}
              ...
              <div>
                ...
                <input
                    type="text"
                    id="subject"
                    name="user_subject" // Add name attribute
                    className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                    id="message"
                    name="user_message" // Add name attribute
                    rows={6}
                    className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500"
                    required
                ></textarea>
